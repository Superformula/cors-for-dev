...
function mapCookieHeader(header, mapCookie) {
  if (!header) {
    return header;
  }
  if (typeof header === 'string') {
    return mapCookie(header);
  }
  return header.map(mapCookie);
}
...
function withCORS(headers, request) {
  ...
  if (request.corsAnywhereRequestState.useWildcard) {
    headers['access-control-allow-origin'] = '*';
  } else {
    // Skip CORS headers if it's same-origin request
    if (!origin) {
      return headers;
    }
    if (headers['x-final-url'] && headers['x-final-url'].indexOf(origin + '/') === 0) {
      return headers;
    }
  ...
  if (request.corsAnywhereRequestState.allowCredentials === true) {
 ...
 * @param req.corsAnywhereRequestState.useWildcard {boolean}
 ...
 * @param req.corsAnywhereRequestState.mapCookie {function}
...
function onProxyResponse(proxy, proxyReq, proxyRes, req, res) {
  ...
  // Strip cookies
  if (requestState.allowCredentials !== true) {
    delete proxyRes.headers['set-cookie'];
    delete proxyRes.headers['set-cookie2'];
  } else {
    proxyRes.headers['set-cookie'] = mapCookieHeader(
      proxyRes.headers['set-cookie'],
      req.corsAnywhereRequestState.mapCookie
    );
    proxyRes.headers['set-cookie2'] = mapCookieHeader(
      proxyRes.headers['set-cookie2'],
      req.corsAnywhereRequestState.mapCookie
    );
  }

  // Capture the response from the backend, check if response is being chunked,
  // and if yes, gather all chunks and pass it as unchunked.
  // TODO Add an unit test for it
  if (req.corsAnywhereRequestState.removeChunkTransferEncoding) {
...
var getHandler = exports.getHandler = function getHandler(options, proxy) {
  var corsAnywhere = {
    /**
     * If true, `Access-Control-Allow-Origin` in the response will be equal to `*`.
     * Otherwise, `Access-Control-Allow-Origin` will be equal to the request's origin (if given).
     * Recommended is to set it as `false`, because some browsers (Firefox) might not like the wildcard;
     * also some crossorigin requests (i.e. with Allow-Credentials) cannot use wildcard neither.
     */
    useWildcard: true,
     ...
     * Accepts string and RegExp values.
     */
    originWhitelist: [],
    ...
    /**
     * Some servers (i.e. lambda servers) might have problems with sending chunked responses.
     * If set to true, we will unchunk such response and serve it as if it hasn't been sent as chunks at all.
     */
    removeChunkTransferEncoding: false,
    /**
     * If you're running your proxy server on a path,
     * like `http://localhost:3000/proxy/https://google.com`,
     * then you can supply it here like `path: 'proxy/'`,
     * so when we'll be getting the request URL,
     * it will be subtracted from the beginning of the URL
     * (`https://google.com` instead of `proxy/https://google.com`).
     */
    path: undefined,
    /**
     * Map cookie string if you want to tweak it, i.e. to remove `Secure` flag or change the cookie path
     * @param {String} cookie
     * @example
     * ```
     * mapCookie(cookie) {
     ...
     *     .replace(/\bSecure\s*;/gi, '')
     *     .replace('/some-path/', '/')
     * }
     * ```
     */
    mapCookie: function mapCookie(cookie) { return cookie; },
  ...
  return function(req, res) {
    req.corsAnywhereRequestState = {
      useWildcard: corsAnywhere.useWildcard,
      getProxyForUrl: corsAnywhere.getProxyForUrl,
      maxRedirects: corsAnywhere.maxRedirects,
      corsMaxAge: corsAnywhere.corsMaxAge,
      allowCredentials: corsAnywhere.allowCredentials,
      mapCookie: corsAnywhere.mapCookie,
      removeChunkTransferEncoding: corsAnywhere.removeChunkTransferEncoding,
    };
    ...
    if (corsAnywhere.path && url.indexOf(corsAnywhere.path) === 0) {
      url = url.replace(corsAnywhere.path, '');
    }
    ...
    if (origin && corsAnywhere.originBlacklist.indexOf(origin) >= 0) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The origin "' + origin + '" was blacklisted by the operator of this proxy.');
      return;
    }

    if (origin && corsAnywhere.originWhitelist.length > 0) {
      var whitelisted = !!corsAnywhere.originWhitelist.find(function (filter) {
        return origin === filter || !!(filter.test && filter.test(origin));
      });
      if (!whitelisted) {
        res.writeHead(403, 'Forbidden', cors_headers);
